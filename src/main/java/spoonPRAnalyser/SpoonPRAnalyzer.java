					...
					if (new Date().getTime() - PR.getCreatedAt().getTime() <= 10000) {
						...
						for (GHPullRequestFileDetail a : GetFiles(pullRequests.get(pullRequestIndex))) {
							...
							if (a.getFilename().endsWith(".java")) {
								getActions(i);
								nbNewMethod += MethodsElements.GetNumberOfNewMethodInFile(actions);
								nbDeletedMethod += MethodsElements.GetNumberOfDeletedMethodInFile(actions);
								nbModifiedMethod += MethodsElements.GetNumberOfModifiedMethodsInFile(actions);
								nbNewTest += TestsElements.GetNumberOfNewTestInFile(actions);
								nbDeletedTest += TestsElements.GetNumberOfDeletedTestInFile(actions);
							}
						}
